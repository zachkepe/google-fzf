document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Handle new keyboard shortcuts: Ctrl/Cmd + K for next, Ctrl/Cmd + Shift + K for previous
  document.addEventListener('keydown', async (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
          e.preventDefault();
          if (e.shiftKey) {
              // Previous match (Ctrl/Cmd + Shift + K)
              const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
              await chrome.tabs.sendMessage(tab.id, { type: 'PREV_MATCH' });
          } else {
              // Next match (Ctrl/Cmd + K)
              const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
              await chrome.tabs.sendMessage(tab.id, { type: 'NEXT_MATCH' });
          }
      }
  ...
  // Removed the Enter/Shift+Enter shortcuts since we're using Ctrl/Cmd + K now
